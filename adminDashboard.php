...
if(isset($_POST['add_staff'])) {
    ...
    $pass        = mysqli_real_escape_string($conn, $_POST['pass'] ?? ''); // keep as-is to avoid breaking login
    $name        = mysqli_real_escape_string($conn, $_POST['name'] ?? '');
    $designation = mysqli_real_escape_string($conn, $_POST['designation'] ?? '');
    $gender      = mysqli_real_escape_string($conn, $_POST['gender'] ?? '');
    $dob         = mysqli_real_escape_string($conn, $_POST['dob'] ?? '');
    $doj         = mysqli_real_escape_string($conn, $_POST['doj'] ?? '');
    $salary      = filter_var($_POST['salary'] ?? null, FILTER_VALIDATE_INT);
    $mobile      = mysqli_real_escape_string($conn, $_POST['mobile'] ?? '');
    $email       = filter_var($_POST['email'] ?? '', FILTER_VALIDATE_EMAIL);
    $credits     = 0; // integer

    // Validate required fields
    if(!$staff_id || !$name || !$designation || !$gender || !$dob || !$doj || $salary === false || !$mobile || !$email) {
        ...
        if ($check_stmt) {
            ...
            if(mysqli_stmt_num_rows($check_stmt) > 0) {
                ...
                if($stmt) {
                    // types: s s s s s s i s s i s  (11 params)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST" action="" onsubmit="return confirm('Are you sure you want to delete this staff member?');">
                                                <input type="hidden" name="staff_id" value="<?php echo htmlspecialchars($staff['StaffID']); ?>">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <?php if ($feedback_result && mysqli_num_rows($feedback_result) > 0): ?>
                                    ...
                                    <tr>
                                        <td><?php echo htmlspecialchars($feedback['F.No'] ?? ($feedback['id'] ?? '')); ?></td>
                                    ...
                                    <?php endwhile; ?>
                                <?php else: ?>
                                    <tr><td colspan="4" class="text-center text-muted">No feedback found.</td></tr>
                                <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
